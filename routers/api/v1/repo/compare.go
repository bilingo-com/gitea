...
func getCompareDiff(ctx *context.APIContext, headUser *models.User, headRepo *models.Repository, headGitRepo *git.Repository, compareInfo *git.CompareInfo, baseBranch string, headBranch string, baseIs, headIs map[string]bool) {
	...
	headCommitID, baseCommitID, diff, commitCount, headTarget, nothingToCompare, hasErr := prepareOldCompareDiff(ctx, headUser, headRepo, headGitRepo, compareInfo, baseBranch, headBranch, baseIs, headIs)
	if hasErr {
		return
	}
	...
	if len(rawBuffer.Bytes()) > MAX_DIFF_LEN {
		compare.Diff.Raw = string(rawBuffer.Bytes()[:MAX_DIFF_LEN])
	} else {
		compare.Diff.Raw = rawBuffer.String()
	}
